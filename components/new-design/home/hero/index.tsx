...
const Hero: React.FC<HeroProps> = async ({ page = 'home' }) => {
  ...
  const desktopImage = heroImages?.find((img: { deviceType: string }) => img.deviceType === 'DESKTOP')
  const mobileImage = heroImages?.find((img: { deviceType: string }) => img.deviceType === 'MOBILE')
  
  // Use database images or fallback to defaults
  // NOTE: Do NOT pre-transform URLs! The custom ImageKit loader handles all transformations
  // This ensures preloaded URLs match the actual rendered URLs for optimal LCP
  const heroDesktop = desktopImage ? {
    src: desktopImage.imageUrl, // Raw URL - loader will transform
    alt: desktopImage.alt,
    width: desktopImage.width || 1920, // Intrinsic width for aspect ratio
    height: desktopImage.height || 1080,
  } : DEFAULT_HERO_IMAGE.desktop
  
  const heroMobile = mobileImage ? {
    src: mobileImage.imageUrl, // Raw URL - loader will transform
    alt: mobileImage.alt,
      ...
      <div className='w-full overflow-x-clip relative'>
        ...
        <div className='hero-lcp-bg hidden lg:block -top-20'>
          <Image
            ...
            // Disable lazy loading behavior that could delay LCP
            loading="eager"
          />
        </div>
        
        {/* Mobile Hero Background Image - optimized for fast LCP */}
        <div className='hero-lcp-bg lg:hidden'>
          <Image
            src={heroMobile.src}
            alt={heroMobile.alt}
            fill
        ...
        <div className='hero-lcp-content w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 2xl:px-0 pt-28 sm:pt-32 lg:pt-36 pb-0 md:pb-68'>
          <div className='relative text-white dark:text-dark text-center md:text-start'>
            ...
            <p className='text-white lg:text-dark lg:dark:text-white text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl 2xl:text-9xl font-semibold -tracking-wider md:max-w-[55%] lg:max-w-45p mt-3 sm:mt-4 mb-5 sm:mb-6 leading-tight' role="heading" aria-level={2}>

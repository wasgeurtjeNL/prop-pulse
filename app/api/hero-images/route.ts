...
function generateHeroAlt(page: string, deviceType: string): string {
  const pageNames: Record<string, string> = {
    home: "PropPulse Thailand - Premium Real Estate Investment",
    properties: "Browse Premium Properties in Thailand",
    contact: "Contact PropPulse Real Estate Experts",
    about: "About PropPulse - Your Trusted Real Estate Partner",
  };
...
export async function POST(request: Request) {
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const sizeMap: Record<string, "1792x1024" | "1024x1792" | "1024x1024"> = {
        DESKTOP: "1792x1024",
        MOBILE: "1024x1792", // Portrait for full-screen mobile background
        TABLET: "1024x1024",
      };
      ...
      const response = await openai.images.generate({
        ...
        prompt: imagePrompt,
        n: 1,
        size: sizeMap[deviceType] || "1792x1024",
